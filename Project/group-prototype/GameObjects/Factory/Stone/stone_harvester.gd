...
@export var generation_freq = 2.0  # Timer interval in seconds
@export var level = 1
@export var cost_to_upgrade = 300
...
func _on_timer_timeout() -> void:
	if current_cart and current_cart.inventory.size() < current_cart.max_inventory_space:
		current_cart.inventory.append(stone)
	else:
		$Timer.stop()
...
func _on_level_upgrade_pressed() -> void:
	if $"../../..".currency >= cost_to_upgrade and level < 10:
		...
		cost_to_upgrade = roundi(cost_to_upgrade * 1.7)
